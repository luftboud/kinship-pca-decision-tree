...
def main():
    ...
    model, mean, std, model_name = train_model(train_embeddings, pairs, labels, family_labels)

    test_pairs, test_labels, test_embeddings = test_preparation.get_prepared_test_data(wk, centering)
    accuracy, report, matrix = test_model(model, mean, std, test_embeddings, test_pairs, test_labels, model_name)
